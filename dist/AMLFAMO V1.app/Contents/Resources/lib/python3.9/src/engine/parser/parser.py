...
class GCodeParser:
    ...
    def _process_motion(self, command: str, params: Dict[str, float], original_line: str, comment: str, line_number: int) -> Move:
        ...
        target_x = self.current_x if self.current_x is not None else 0.0
        target_y = self.current_y if self.current_y is not None else 0.0
        target_z = self.current_z if self.current_z is not None else 0.0
        ...
        # If the coordinates were unknown, assume we teleported to the target
        if self.current_x is None: self.current_x = target_x
        if self.current_y is None: self.current_y = target_y
        if self.current_z is None: self.current_z = target_z
    ...
    def _create_comment_move(self, original_line: str, comment: str, line_number: int) -> Move:
        ...
        self.move_id_counter += 1
        pt = Point3D(self.current_x or 0.0, self.current_y or 0.0, self.current_z or 0.0)
        return Move(
            id=self.move_id_counter,
